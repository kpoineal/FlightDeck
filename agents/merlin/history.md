...
### 2026-09-04 — Inbox Refactor Regression and Coverage Review

**Baseline:** `47bb04831a037074f1f81b52b4d2693dd346c6d9`, the first parent of mailbox merge `215fc1f7d98eef543b220b0b1beab0f671edbab3`. The similarly named `ba08673` is not a valid pre-refactor baseline because its tree matches the merge.

**Validation:** The combined `npm test` process was interrupted by the execution environment before TAP summary. Running the same 15 package-listed files individually with `node --test --test-reporter=tap` produced 299 tests: 295 passed, 0 failed, 4 skipped. Unlisted `test/sort-utils.test.js` passed 20/20. A Vite renderer build to `%TEMP%` passed with 119 event-directive deprecations, 67 accessibility diagnostics, and 6 unused diagnostics.

**Durable coverage findings:**
- `src/preload.js` omits `APP_RESUMED` from its local channel map while subscribing with it; a direct probe observed `ipcRenderer.on(undefined, ...)`. The preload contract test never calls `onAppResumed`.
- The pre-refactor Radar wired item move, inline field edits, monitor-prompt edits, and suggested-step actions. The merged main Radar no longer exposes those paths. `ScannerSection`, `TrackerCard`, `TrackerRow`, and `EditableField` form an unreachable component cluster; `MailboxView` is also runtime-unreachable but remains referenced by a source-string test.
- Core mailbox predicates, action-proposal transitions, deletion transactions, navigation helpers, and scanner exclusion logic have focused unit coverage. Mounted Svelte behavior, scanner settings, the monitoring loop/timers, popout interactions/deletion failure handling, and full renderer→preload→main round trips do not.
- Several Radar tests assert source regexes rather than behavior, so they can stay green while event wiring, focus behavior, modal flows, or runtime reachability breaks. `sort-utils.test.js` is omitted from `npm test`.

**Characterization priority:** First lock preload/main channel parity and app-resume behavior; then mount Radar to preserve or consciously retire move/edit/prompt/next-step workflows; then cover ScheduleControls plus due-item timers, popout persistence failures/state sync, scanner settings CRUD, and proposal/prompt round trips before deleting unreachable components or persisted settings state.

### 2026-09-08 - Inbox Parity Batch 1 Red Characterization

**Scope:** Added test-only characterization for the verified preload, lifecycle, proposal-context, KPI, and mounted Inbox defects. No product, prompt, schema, package, documentation, or configuration file was changed.

**Files changed:**
- `test/main-ipc-handlers.test.js`
- `test/mailbox.test.js`
- `test/proposal-synthesis.test.js`
- `test/radar-selection-transition.test.js`
- `scripts/test-radar-browser.js`

**Coverage added:**
- Preload `onAppResumed` uses canonical `APP_RESUMED`, forwards the payload, and removes the identical listener.
- `normalizeItem` preserves the `Inbound` status label while mapping lifecycle to `in-progress`.
- Proposal context emits `thread.lastChangedAt`, prefers the canonical item field, falls back to `updatedAt` then `lastUpdatedAt`, and keeps the newest six entries from newest-first history.
- KPI blocked count reads `lifecycleStatus === 'blocked'` regardless of severity and ignores the legacy `isBlocked` flag.
- Mounted-source contracts require the Add Item path to import/use `normalizeItem`, Today urgent work to read canonical blocked lifecycle, Teams review to preserve/display `why`, `risk`, and `reviewNote`, and Radar detail to surface completion confidence.
- The existing deterministic Teams browser fixture now supplies nonempty risk metadata and requires all three review fields to be visible before send.

**Focused Node validation:**
- Command: `node --test test/main-ipc-handlers.test.js test/mailbox.test.js test/proposal-synthesis.test.js test/radar-selection-transition.test.js`
- Result: 67 tests, 59 passed, 8 expected failures, 0 skipped.
- Expected failures: undefined preload channel instead of `app-resumed`; `Inbound` remains `unknown`; KPI counts `isBlocked`; proposal context omits canonical `lastChangedAt` and selects the wrong history slice; RadarView omits the `normalizeItem` import; Today reads `isBlocked`; Teams draft drops review metadata; completion confidence is absent from Radar detail.
- Existing Inbox chronological/Recent assertions remained green. No filter tests were added.

**Focused browser validation:**
- Command: `FLIGHTDECK_BROWSER_SCENARIO=proposal-teams-review-only npm run test:radar:browser` (PowerShell environment assignment used locally).
- Renderer build passed in 575 ms.
- Result: 1 scenario, 0 passed, 1 expected failure. The real Teams draft mounted successfully, then timed out waiting for `A confirmed window is the smallest step that unblocks the renewal.` No browser startup, syntax, page-error, or harness failure occurred.

**Contract ambiguity:** None blocking. `lastChangedAt` is characterized as the canonical output field while retaining `updatedAt` and `lastUpdatedAt` as input fallbacks.

### 2026-09-08 - Scanner Deletion Batch 2 Red Contracts

**Scope:** Added the test-first scanner deletion transaction suite and registered it in the explicit package test list. No product, prompt, style, documentation, or configuration file was changed.

**Files changed:**
- `test/scanner-deletion.test.js`
- `package.json`

**Coverage added:**
- Stable preview identity, unique hot/cold/combined item IDs and counts, linked/no-effect/effectful proposal IDs and counts, deduplicated audit IDs, prospective tombstone IDs, impacted selections, deterministic preview token, and valid reassignment targets including Unassigned Inbox.
- Reassignment to `null` or another scanner while preserving lifecycle, proposal/effect/history links, selections, and tombstones.
- Delete-all removal of owned hot/cold items and no-effect proposal administration, tombstoning of unique IDs, preservation of confirmed/uncertain audit evidence, and selection cleanup.
- Strict cold and canonical receipt acceptance, cold rollback after canonical failure, recovery-required ambiguity, and stable rollback of transaction-owned records.
- Fail-closed stale preview, invalid destination, missing scanner, overlapping transaction, active scanner operation, and active affected-item operation behavior.
- Preservation of unrelated concurrent hot/cold/proposal/history/tombstone/selection mutations with live cold-snapshot retry.

**Immediate focused validation:**
- Command: `node --test test/scanner-deletion.test.js`
- Result: exit 1 solely because `src/svelte/lib/scanner-actions.js` is absent. No parser, hook, or fixture failure was reported.

**Registered package validation:**
- Command: `npm test`
- Result: exit 1; 323 tests discovered, 303 passed, 0 failed assertions, 16 new scanner tests cancelled by the missing-module suite hook, 4 skipped, 0 todo. No suite outside `test/scanner-deletion.test.js` failed.

**Contract assumptions:** Stable codes are `NOT_FOUND`, `INVALID_TARGET`, `STALE_PREVIEW`, `BUSY`, `PERSISTENCE_FAILED`, and `PERSISTENCE_RECOVERY_REQUIRED`; success includes `code: 'OK'`; retained effectful proposals are marked `auditOnly: true`; preview tombstones are the prospective unique delete-all item IDs.

**Deferred:** Browser scanner-deletion scenarios remain deferred until the public transaction API and deletion modal exist; adding selectors or UI timing against absent surfaces would not produce a discriminating red test.

### 2026-09-08 - Batch 3 Inbox Filters and Control Contracts

**Scope:** Added test-first contracts for Inbox filtering, deterministic Recent/Priority ordering, mounted filter/control behavior, scanner deletion browser acceptance, and a representative Recent/filter/edit workflow. No product source, styles, prompts, docs, or config were changed.

**Files changed:**
- `test/inbox-filters.test.js`
- `test/mailbox.test.js`
- `test/radar-selection-transition.test.js`
- `scripts/test-radar-browser.js`
- `package.json`
...
- Quick filters: unread, new, updated, critical, blocked, due-soon.
- Refine facets: severity, lifecycle, scanner, read, monitoring, due, signal, activity-age.
- Empty facets mean any; values OR within a facet; quick filter and populated facets AND together.
- Due-soon includes overdue dates through seven calendar days; signal matching normalizes source and evidence signal types.
- Filtering preserves input ordering and does not mutate the source array.

**Mounted/source contracts:**
- Recent defaults to strict activity-descending order with stable item-ID tie break; Priority remains unread/update, risk, severity, recency, then ID.
- Sort, quick filter, Refine facets, search, smart view, and scanner scope remain independent and compose.
- Active filter chips are removable; rows support ArrowUp, ArrowDown, Home, and End without implicitly marking read.
- Scanner assignment, due date, owner, done criteria, NEW/UPDATED, snooze, monitor-paused, scanner counts/status/last-next run, and run-now remain reachable.
- Advanced or popout retains stored monitor prompt, detailed schedule, signals, and notifications without prompt regeneration.

**Validation:**
- First command: `node --test test/inbox-filters.test.js` -> 1 test, 0 passed, 1 failed solely with `ERR_MODULE_NOT_FOUND` for `src/svelte/lib/inbox-filters.js`.
- Focused existing tests: 48 tests, 43 passed, 5 expected failures, 0 skipped. Failures are Priority comparator semantics plus four mounted Inbox contracts.
- Full registered Node suite: 330 tests, 320 passed, 6 expected failures, 4 skipped, 0 todo. No unrelated suite failed.
- Renderer build passed with existing Svelte deprecation/unused-selector warnings.
- Browser scanner deletion: preserve/reassign and delete-all both passed after reload; 2 scenarios passed, 0 failed, with zero external requests, page/console errors, overflow, or pane overlap.
- Browser Recent/filter/edit: 1 scenario, 0 passed, 1 expected failure at the first product contract: sort value was `priority`, expected `recent`; harness metrics remained clean.

### 2026-09-08 - Browser Durability Barrier Repair

**Root cause:** The `inbox-recent-filter-edit-workflow` persistence barrier used an async `page.waitForFunction` predicate. The returned promise satisfied Playwright before the queued browser-bridge write reached `__flightdeck_browser_acceptance__:flightdeck.demo.v2`, allowing reload to race persistence.

**Repair:** Replaced only that scenario's predicate with a synchronous localStorage parse and poll for the expected scanner assignment, owner, done criteria, and unchanged monitor prompt.

**Validation:**
- Command: `$env:FLIGHTDECK_BROWSER_SCENARIO='inbox-recent-filter-edit-workflow'; npm run test:radar:browser`
- Result: renderer built in 545 ms; 1 scenario passed, 0 failed; `ALL_RADAR_BROWSER_SCENARIOS_PASS`.
- `node --check scripts/test-radar-browser.js`: passed.
- `git diff --check -- scripts/test-radar-browser.js`: passed.

**Remaining harness debt:** 12 other async `waitForFunction` predicates remain intentionally unchanged and should be audited separately.

### 2026-09-08 - Scanner and Monitor Prompt Golden Contract

**Scope:** Added deterministic, explicit-string characterization tests for the tuned scanner and monitor prompt builders. No prompt, schema, production source, browser script, documentation, or configuration semantics changed.

**Files changed:**
- `test/prompt-fidelity.test.js`
- `package.json`
- `.squad/agents/merlin/history.md`

**Coverage locked:**
- Scanner mission and block ordering, fixed last-run boundary interpolation, filtered signal sources, same-scanner title cleanup/dedup, active cross-scanner exclusions, exact evidence URL/signal-time schema metadata, due and observable-done rules, and 0-2 concrete next steps.
- Monitor temporal boundary, stored prompt/context, prior summary and evidence URLs, owner/people/due/done metadata, filtered signals, two newest history summaries only, complete response schema, exact `hasNewInfo: false` copy-verbatim/status semantics, and empty next steps on no update.
- Representative schedule, notification, work-hours, and dedup settings remain control-plane metadata and are not silently injected into generated prompt text.

**Validation:**
- `node --test test/prompt-fidelity.test.js`: 2 tests, 2 passed, 0 failed, 0 skipped.
- `node --test test/proposal-synthesis.test.js test/scanner-exclusion.test.js test/mailbox-actions-monitor.test.js test/prompt-fidelity.test.js`: 33 tests, 33 passed, 0 failed, 0 skipped, 0 cancelled, 0 todo.
- `git diff --check`: passed.
- `git diff --exit-code -- src/prompts src/svelte/lib/prompts.js src/svelte/lib/constants.js`: exit 0; protected prompt/schema paths unchanged.