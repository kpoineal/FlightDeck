...
### 2026-09-04 — Post-Inbox Backend/IPC Reachability Audit

**Baseline:** `47bb04831a037074f1f81b52b4d2693dd346c6d9`, the first parent of PR #160's mailbox-thread-view merge (`215fc1f7d98eef543b220b0b1beab0f671edbab3`). Current allowed application/test paths match that merge; the inbox implementation entered in `ba086735bbe37b31c8e0ef5a28587c7e7e94c7d6`.

**Durable findings:**
- Main/preload IPC remains string-duplicated. `APP_RESUMED` exists in the shared contract and main sender but is absent from preload's local channel map; startup therefore subscribes on `undefined`, and the IPC test omits `onAppResumed`.
- The live Inbox is a smart view inside `RadarView`; standalone `MailboxView.svelte` and `ComposeDraftModal.svelte` were added but never imported by any production entrypoint.
- `PROBE_WORKIQ_MCP` was added with the inbox work but has no production renderer caller. Connection state still reflects PTY EULA acceptance, while MCP-backed synthesis/draft/message availability is discovered only on action failure.
- Radar Add Item calls `normalizeItem()` without importing it, so the visible create-item path fails at submit time despite the modal being reachable.
- Scanner deletion bypasses transactional `deleteItem()`: it removes only hot items, leaves cold-store items and linked action proposals, ignores persistence confirmation, and nevertheless logs that the scanner and its items were deleted.
- The old ScannerSection/TrackerCard/TrackerRow subtree is no longer imported. Main-window controls for density, per-scanner sort, moving items, owner/due/done-criteria edits, and monitor-prompt editing disappeared; prompt editing remains available in popouts.
- `storeGetAll`, `storeGetSize`, `storeMigrateFromLocalStorage`, and `openMarkdownWindow` are legacy compatibility bridge surfaces with no production Svelte caller. `probeMcp` is a newer likely orphan. PTY exports and WorkIQ MCP action methods are otherwise live through concrete IPC call chains.
- Persistence keeps v1-to-v2 item normalization plus cold-store eviction and transactional card/proposal deletion. Legacy-key migration still deletes v1 before a confirmed v2 write, relying on later autosave.

**Validation:** preload probe observed an undefined app-resume channel; IPC tests passed 10/10; focused integration/persistence/action/navigation tests passed 227 with 4 skipped; renderer build exited 1 without diagnostics under local Node v26.2.0.

### 2026-09-08 — D2 Scanner Deletion and D4 Evidence-Only Cleanup

**Durable findings:**
- Current scanner deletion removes the scanner definition and hot items only, then starts an unawaited save. It does not transact cold items, linked proposals, deleted-ID exclusions, selection, or rollback.
- The per-item deletion transaction is the correct behavioral model, but scanner deletion must use one aggregate transaction rather than sequential item deletions to avoid partial success.
- Recommended D2 policy: an impact-preview dialog with `Keep and reassign` as the default and `Delete all` as the explicit destructive choice. Reassignment should support both unassigned Inbox (`scannerId: null`) and another scanner.
- `Delete all` must tombstone every deleted thread, remove only no-effect proposals and administrative events, and retain confirmed or uncertain external-action evidence as audit-only records. Accepted canonical and cold-store persistence receipts define success; otherwise rollback or require reload recovery.
- D4 evidence-only work may inventory current reachability and risk now, but it changes nothing, authorizes no deletion, and must be refreshed after Inbox parity restoration before a named deletion set can receive separate approval.

**Pending policy proposal:** `.squad/decisions/inbox/viper-d2-safe-scanner-deletion-policy-2026-09-08.md`.

### 2026-09-08 — Batch 1 Core Contract Repairs

**Scope completed:** Repaired four characterized contract defects without changing public bridge APIs, prompt files, schema text, or mounted UI.

**Changes made:**
- Added `APP_RESUMED: 'app-resumed'` to preload's sandbox-safe local IPC channel catalog so `onAppResumed` registers and removes the canonical listener.
- Normalized scanner status `Inbound` to the existing `in-progress` lifecycle while preserving the original status label.
- Proposal synthesis now emits canonical `lastChangedAt`, accepts `updatedAt` and `lastUpdatedAt` as legacy fallbacks, canonicalizes valid timestamps to ISO, and keeps the first six entries from newest-first history.
- KPI blocked count now derives from `lifecycleStatus === 'blocked'` rather than the obsolete `isBlocked` flag.

**Validation:** Focused Viper suite passed 46/46 with no failures or skips. Authorized product-file `git diff --check` passed. Separate radar transition characterization passed 17/21 with four expected Goose-owned UI failures: Add Item `normalizeItem` import, Today blocked lifecycle consumption, Teams rationale/risk/review display, and completion-confidence display.

### 2026-09-08 - Scanner Deletion and Persistence Performance Repair

**Scope completed:** Optimized scanner deletion preview/execute and canonical save scheduling without changing public APIs, transaction receipts, rollback behavior, or active-operation conflict handling.

**Implementation learnings:**
- Scanner deletion now indexes affected items and proposal-linked thread/global events in deterministic source order, then uses Set/Map membership for proposal classification, audit extraction, selection checks, and preview-token record filtering.
- Ordinary canonical saves use per-storage-target durability generations. Same-tick bursts coalesce to the latest generation; each waiter resolves from the write that covers its requested generation.
- A transaction fences new ordinary saves, waits for the pre-existing ordinary drain, and records later requests as strict follow-up generations. Active or nested transactions still fail closed with `BUSY`.
- Successful primary or rollback writes settle only generations captured before those writes, avoiding redundant follow-ups while ensuring mutations arriving during a write receive another durable write.

**Validation:** Scanner deletion contract passed 16/16. Focused persistence/item/scanner/mailbox run passed 136/136 across 21 suites. Renderer build passed with 195 modules in 608 ms. Fifty same-tick saves with 10 ms injected latency produced one write, all 50 promises resolved true, and the final snapshot was current. A transaction started behind an active ordinary write completed successfully.

**10k benchmark:** At 500 hot plus 9,500 cold items, preview measured 18.84 ms median / 33.56 ms p95; reassign execute 25.47 / 28.75 ms; delete-all execute 25.03 / 26.71 ms. Slider's prior production-shaped measurements were approximately 479-653 ms preview and 493-656 ms execute.

### 2026-09-08 — Batch 2 Scanner Deletion Transaction

**Scope completed:** Implemented the DR-5/DR-6 scanner deletion library contract without changing UI, prompt, schema, test, script, package, or configuration files.

**Changes made:**
- Added `scanner-actions.js` with deterministic impact previews and token-validated `reassign` / `delete-all` transactions.
- Added `operation-guards.js` with synchronous multi-key leases; scanner runs, item checks, and scanner deletion now fail closed on overlapping scanner/item operations.
- Reused item deletion's strict cold-store receipt checks and stable-position rollback helpers.
- `delete-all` tombstones unique hot/cold IDs, removes only no-effect proposals and administrative events, preserves effectful proposals as durable `auditOnly` evidence, clears only removed selections, and records one scanner history event.
- Rollback restores transaction-owned state while retaining unrelated concurrent mutations; ambiguous rollback reports `PERSISTENCE_RECOVERY_REQUIRED`.
- Persistence now preserves the `auditOnly` marker through save/hydration normalization.

**Goose handoff:** The deletion UI should preview immediately before execution, default to reassignment, pass the returned token unchanged, offer only `reassignmentTargets`, refresh on `STALE_PREVIEW`, disable/retry on `BUSY`, and require reload on `PERSISTENCE_RECOVERY_REQUIRED`.

**Validation:** `test/scanner-deletion.test.js` passed 16/16. The five requested affected suites passed 84/84. Combined focused validation passed 100/100 across 13 suites. Editor diagnostics found no errors in the six touched libraries, and `git diff --check` exited 0.

### 2026-09-08 — Batch 3 Canonical Persistence Ordering

**Scope completed:** Serialized canonical saves in `src/svelte/lib/persistence.js` and made cold-storage eviction reconcile against the live item store rather than restoring a pre-await snapshot.

**Queue semantics:**
- `savePersistentState()` appends canonical writes to one promise queue, so accepted `storeSet` calls complete in mutation order and callers may await the boolean receipt without changing the public API.
- Queue failures are converted to `false`, preventing fire-and-forget callers from producing unhandled rejections.
- A queued save that reaches an active strict transaction joins its existing follow-up receipt instead of overlapping the transaction.
- Item store mutations remain synchronous; only persistence work is serialized.
- Cold and active-cap eviction remove live records by ID after awaited cold persistence, then re-snapshot current items before the canonical write. This preserves concurrent owner/due/done/scanner edits instead of overwriting them with a stale pre-await array.

**Validation:** Immediate mailbox/persistence gate passed 27/27 after each substantive edit. Focused item deletion, persistence deletion, scanner deletion, mailbox, mailbox monitor, and scanner exclusion suites passed 72/72. Renderer build passed. Authorized product-file `git diff --check` passed, and editor diagnostics reported no persistence errors.

**Remaining browser handoff:** `inbox-recent-filter-edit-workflow` still fails 0/1 at the final owner assertion with persisted value `InitialBrowser owner`; all page, console, external-request, overflow, and overlap metrics remain zero. The value concatenation indicates the controlled `on:change` owner editor can be reset during Playwright `fill()` by an item-store rerender triggered by unrelated cold eviction. Goose should rerun and repair the mounted editor interaction; the persistence layer must not duplicate component-level input choreography.

**Final correction:** Cold eviction and active-cap enforcement now affect only the persisted snapshot after accepted cold-store receipts; they no longer mutate the live `items` store after an asynchronous boundary. This removes persistence-driven reactive rerenders while keeping current session objects intact and applying the hot/cold split on reload. The same browser scenario still reports `InitialBrowser owner`, so the remaining controlled-input concatenation is not caused by persistence mutating the item store and remains a Goose-owned mounted editor issue. Final focused validation passed 72/72 across 14 suites; renderer build completed in 591 ms with zero errors and existing Node module-type, Svelte stale-capture, and deprecated-event warnings; final `git diff --check` passed.
