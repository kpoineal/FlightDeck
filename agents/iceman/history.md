...
### 2026-09-04 - Inbox Product Contract Consolidation

- The inbox-driven Radar is the intended product direction; commit `47bb04831a037074f1f81b52b4d2693dd346c6d9` is the capability/settings parity baseline, not a layout rollback target.
- `f2497e53117828515be111300312c7fadda735b2` descends from `47bb048` and has byte-identical audited prompt surfaces, so it is a valid prompt-only comparison point.
- Verified critical defects: preload references an undeclared `APP_RESUMED` key, Add Item calls unimported `normalizeItem`, and scanner deletion removes hot items but not cold items or linked action proposals.
- Verified parity gaps in the mounted inbox include item reassignment, editing due/owner/done criteria and monitor context, scanner telemetry/filtering, distinct NEW vs UPDATED state, and explicit snooze/monitor-paused context.
- Prompt safety is an end-to-end metadata lifecycle, not template text alone: scanner output -> normalization -> persistence/hydration -> monitoring/history -> proposal synthesis -> human review/action. Characterize every stage before changing semantics.
- Seven runtime-unreachable component candidates require proof and user approval before deletion: `ScannerSection`, `TrackerCard`, `TrackerRow`, `EditableField`, `MailboxView`, `ComposeDraftModal`, and `PromptEditor`.
- Proposed contract and decision-ready backlog: `.squad/decisions/inbox/iceman-inbox-product-contract.md` (pending user confirmation).

### 2026-09-08 - Inbox Decision Clarifications

- D1 is approved: Inbox defaults to strict Recent ordering, while filtering remains a separate membership operation; Priority remains an alternate sort.
- Recommended filter model builds on mounted Smart Views, scanner scope, and search, with compact attention, severity, and work-state facets; exact facets remain open.
- D2 remains pending. Scanner deletion should preview impact and distinguish deleting all scanner-owned hot/cold cards from keeping cards through reassignment before scanner removal.
- A delete-all transaction should tombstone removed item IDs, remove only no-effect linked proposals, preserve effectful action/audit evidence, make cancellation mutation-free, and roll back on persistence failure.
- D3 is a placement decision, not a capability-existence question: routine triage must remain reachable in the Inbox; deep prompt/signal/schedule configuration may remain advanced or in popout; scanner operations stay scanner-scoped.
- D4 evidence-only orphan work authorizes reachability, bundle, reference, capability, migration, and before/after validation evidence plus a named deletion proposal after parity; it authorizes no deletion.
- No new decision inbox proposal was added because these clarifications refine the existing pending contract rather than introduce a separate cross-team policy.

### 2026-09-08 - Inbox Parity Product Acceptance

- **Verdict:** Approved with follow-ups. No product blocker remains against the approved Inbox-first direction.
- Recent is the mounted default, Priority is a real alternate comparator, and quick/Refine filters compose independently with smart view, scanner scope, search, and selection.
- Scanner deletion defaults to Keep and reassign, offers Unassigned Inbox and another scanner, and requires explicit acknowledgement for Delete all while preserving action evidence.
- Frequent item/scanner controls, Add Item, lifecycle/blocked handling, proposal rationale/safety, NEW/UPDATED/snooze/paused cues, keyboard navigation, and scanner operations are present in the mounted Inbox; stored monitor prompt editing remains accessible in popout without prompt-path changes.
- No protected prompt/orphan candidate was modified or deleted, and App continues to mount the Inbox-style RadarView rather than the retired card layout.
- **Polish:** Priority is both a smart-view label and sort value while the sort label is visually hidden; make the distinction explicit in visible UI copy.
- **Coverage:** Add Item has source-contract coverage but no mounted submit/persist/reload browser scenario. One full browser run also hit a non-reproducible 760px Action Queue focus-restoration failure; the isolated retry and final full rerun passed.
- **Validation:** Focused Node acceptance slice passed 159/159. Final registered browser suite passed 59/59 with zero page, console, external-request, overflow, or overlap failures; renderer build passed with existing Svelte deprecation warnings.
