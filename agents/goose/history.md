...
### 2026-09-04 — Inbox vs. Radar-Card Capability Parity Audit
- **Historical baseline**: `47bb04831a037074f1f81b52b4d2693dd346c6d9`, the first parent immediately before PR #160 merged the FlightDeck 2.0 inbox baseline at `215fc1f7d98eef543b220b0b1beab0f671edbab3`.
- **Intended direction**: `App.svelte` now mounts the inbox-style `RadarView.svelte`; scanner sections and card/row density modes are intentionally replaced by smart views, a thread list, and a detail pane.
- **Retained**: severity/lifecycle mutation, read/seen semantics, local deletion, popout, activity timeline, people/sources, scanner CRUD, add-item flow, search/notification navigation, and item schedule/signal/run-now controls remain reachable.
- **Verified parity gaps**: no active way to move an existing item between scanners; due/owner/done criteria are read-only; per-scanner severity/status/new/updated filters, completed-date sorting, collapsed header metrics, and direct scanner run are absent or hidden; item monitoring prompt and next-run metadata moved to the popout; explicit NEW/UPDATED counts and snooze-until/paused metadata are degraded.
- **Current control mismatch**: the visible Radar sort selector is ignored in the default Inbox smart view because Inbox always uses chronological `compareInboxThreads` ordering.
- **Orphan candidates**: `MailboxView.svelte`; the unreachable `ScannerSection` -> `TrackerCard`/`TrackerRow` -> `EditableField` component tree; `PromptEditor.svelte`, `MorningBanner.svelte`, `ComposeDraftModal.svelte`, `MomentumGauge.svelte`, `PulseStrip.svelte`, and `HeatmapStrip.svelte`; plus persisted `density`, `filter`, `collapsedSections`, and `scannerSortPrefs` state retained for the old renderer path. Shared mailbox/tracking CSS must be pruned selector-by-selector because active Radar and popout still reuse portions.

### 2026-09-08 — D3 Forensic Control Placement Clarification
- **Baseline split**: item triage controls were distributed across card/row headers, Activity/Overview/Monitor panels, and popout; scanner operations lived in scanner headers/settings; All/Archived and full/minimal density lived in the Radar toolbar. Several values were display-only even then, especially people, evidence, activity, timestamps, and next-run labels.
- **Inbox-native placement**: keep frequent triage in mounted list/detail (distinct NEW/UPDATED/read cues; severity/lifecycle; complete, archive, snooze, delete; move scanner; due, owner, done criteria; scanner status/counts/last-next run and run now). Keep full monitor/scanner prompts, signal selection, detailed schedules, notifications, dedup, missed-run, retention, and exclusion tuning in an advanced disclosure/settings or popout.
- **D1 recommendation**: retain Recent as the default sort and Priority as an explicit alternative. Treat filters separately: quick chips for Unread, NEW, UPDATED, Critical, Blocked, and Due soon; a More filters menu for severity, lifecycle, scanner/source, read state, monitoring state, due window, signal type, and activity age.
- **Retire with card layout**: Activity/Overview/Monitor card tabs, full/minimal density, per-scanner accordion/collapse state, and drag grips as the primary move mechanism. Preserve the capabilities through Inbox controls rather than restoring those presentation mechanics.

### 2026-09-08 — Batch 1 Mounted UI Repair
- **Radar Add Item**: Imported canonical `normalizeItem` in `RadarView.svelte`, binding the existing Add Item creation path without changing normalization or prompt semantics.
- **Today urgent work**: Replaced the obsolete `isBlocked` read with `lifecycleStatus === 'blocked'`.
- **Teams review**: Preserved normalized synthesis `why`, proposal `risk`, and `reviewNote` in `teamsDraft` and conditionally displayed all three before send while keeping recipient/message editing and send controls intact.
- **Completion confidence**: Added a conditional detail metadata row that displays `selected.completionConfidence` only when present.
- **Files changed**: `src/svelte/components/RadarView.svelte`, `src/svelte/components/TodayView.svelte`. No style changes were needed.
- **Validation**: `node --test test/radar-selection-transition.test.js` passed 21/21. Focused `proposal-teams-review-only` browser scenario passed 1/1 with zero page/console/external-request/overflow/overlap failures. `npm run build:renderer` passed; only existing ScannerForm deprecation/accessibility warnings were reported. Authorized-file `git diff --check` passed.

### 2026-09-08 — Batch 2 Scanner Deletion UI Integration
- **Two-phase flow**: Replaced Radar's immediate scanner/item mutation with `previewScannerDeletion()` followed by token-preserving `executeScannerDeletion()`. The UI performs no scanner deletion persistence outside the library contract.
- **Safe default**: Added `ScannerDeletionModal.svelte` with `Keep and reassign` selected by default and Unassigned Inbox (`null`) as the default destination. Preview-provided scanners are available as alternate destinations.
- **Destructive path**: `Delete all` is a separate critical treatment and requires an explicit acknowledgement checkbox before its action enables. The modal has no form-level Enter submission.
- **Impact and recovery**: The modal translates hot/cold/proposal/audit/tombstone data into current Inbox, archived thread, proposed action, action evidence, retained record, and blocked-return counts. Stale previews refresh and reset confirmation; busy, persistence, and invalid-target errors remain open; recovery-required disables mutation and instructs reload.
- **Selection and focus**: Successful deletion clears or replaces removed local selection, preserves reassigned selection, resets scanner/modal state, and relies on the library for highlighted/proposal reconciliation. Cancel, Escape, and backdrop perform no mutation, reopen settings, and restore focus; the modal traps focus while open.
- **Files changed**: `src/svelte/components/ScannerDeletionModal.svelte`, `src/svelte/components/ScannerSettingsModal.svelte`, `src/svelte/components/RadarView.svelte`, and `src/styles/modal.css`.
- **Validation**: First imported-component renderer build passed. Integrated `node --test test/scanner-deletion.test.js test/radar-selection-transition.test.js` passed 37/37 with 0 failures, skips, cancellations, or todos. Final `npm run build:renderer` passed with no warnings or errors. `git diff --check` passed with only unrelated existing LF-to-CRLF notices. Browser scanner-deletion coverage is not present yet and remains for Merlin.

### 2026-09-08 — Batch 3 Inbox Filters and Controls
- **Pure filters**: Added `src/svelte/lib/inbox-filters.js` with the approved quick filters and Refine facets. Values OR within a facet, populated facets and the quick filter AND together, Due soon includes overdue through seven calendar days, signal matching includes item and evidence types, and filtering preserves source order without mutation.
- **Ordering and composition**: Recent is the mounted default with deterministic activity/ID ordering. Priority now orders unread/NEW/UPDATED, risk, severity, recency, and stable ID. Sort, smart view, scanner scope, search, quick filter, and Refine state compose independently.
- **Mounted Inbox**: Added compact quick chips, bounded Refine controls, removable active filters, roving ArrowUp/ArrowDown/Home/End selection without read mutation, distinct NEW/UPDATED/snoozed/monitor-paused cues, scanner counts and run metadata, direct scanner run, and frequent scanner/due/owner/done controls through canonical `setItemField`.
- **Advanced parity**: Existing popout and `ScheduleControls` already preserve the stored monitoring prompt plus detailed schedule, signals, and notifications, so those files required no edit and prompt text remained unchanged.
- **Files changed**: `src/svelte/lib/inbox-filters.js`, `src/svelte/lib/mailbox.js`, `src/svelte/components/RadarView.svelte`, `src/styles/command-center.css`.
- **Validation**: Initial filter test passed 13/13. Final focused command `node --test test/inbox-filters.test.js test/mailbox.test.js test/radar-selection-transition.test.js` passed 61/61 across 8 suites. `npm run build:renderer` passed in 641 ms with zero warnings/errors and existing deprecation notices. `git diff --check` passed with unrelated LF-to-CRLF warnings.
- **Browser result**: `FLIGHTDECK_BROWSER_SCENARIO=inbox-recent-filter-edit-workflow npm run test:radar:browser` built successfully but finished 0/1. The flow reached and applied all editors with zero page, console, external-request, overflow, or overlap failures; after reload, `owner` reverted from `Browser owner` to `Initial owner`.
- **Viper handoff**: Canonical `setItemField` launches unawaited `savePersistentState()` writes. `writePersistentState()` snapshots before asynchronous cold-store work and permits overlapping writes, so rapid scanner/owner/due/done edits can complete out of order and restore stale fields. Viper must serialize/coalesce canonical writes or expose an awaitable mutation receipt that guarantees last-state-wins persistence. Do not repair this with component-level persistence choreography.

### 2026-09-08 — Batch 3 Mounted Editor Local Repair
- **Interaction repair**: Replaced reactive `value` plus separate `on:change` handlers for scanner assignment, due date, owner, and done criteria with Svelte function bindings. Each mounted control now reads the current selected-item value and writes directly through canonical `setItemField` / `updateDueDate`, eliminating the owner clear-and-rerender concatenation path without component-level persistence calls.
- **Focused validation**: After each of three bounded repair attempts, `node --test test/radar-selection-transition.test.js test/inbox-filters.test.js` passed 38/38 across 3 suites. The final `npm run build:renderer` passed with no `RadarView.svelte` diagnostics.
- **Browser result**: The exact `inbox-recent-filter-edit-workflow` still finished 0/1 after the third attempt. The final reload assertion was `actual 'Initial owner'` versus `expected 'Browser owner'`; health metrics remained zero for unexpected external requests, page errors, console errors, desktop/mobile overflow, and desktop/mobile overlap. The mounted concatenation symptom is gone, but the accepted owner value is still replaced during reload outside the authorized component surface.
- **Diff integrity**: `git diff --check` exited 0 with no errors; only unrelated LF-to-CRLF notices were reported.
- **Files changed**: `src/svelte/components/RadarView.svelte` plus this Goose history entry. `src/styles/command-center.css` was not changed.

### 2026-09-08 - Mounted Radar Projection Performance Repair
- **Root cause**: `RadarView.svelte` excluded cold duplicates with `coldItems.filter(...items.some(...))`, making mounted hot/cold projection quadratic. Smart-view counts, scanner counts, visible filtering, and selected-title duplicate detection then rescanned the full projection.
- **Repair**: Replaced nested hot/cold membership checks with a hot-ID `Set`, preserving hot-item precedence and hot-then-cold deterministic order. Added one reactive projection index for all six smart-view counts, scanner counts, and title groups. Combined smart-view, scanner, search, and Inbox filter checks into one visible-thread pass before the unchanged explicit Recent/Priority comparator branches. No memo cache or independently mutable reactive state was added.
- **10k measurement**: Node v26.2.0, 500 hot plus 9,500 non-overlapping cold items, 10 warmups and 100 measured iterations. Isolated nested dedup median/p95 was 4.568/5.291 ms; Set dedup was 0.083/0.273 ms, a 98.18% median reduction. Slider's richer mounted baseline was 47.7 ms total, including 31.5 ms dedup and 4.8 ms repeated count/duplicate scans; the repaired pre-DOM computation is conservatively estimated at about 13-16 ms, dominated by the unchanged 11.3 ms filter plus Recent sort.
- **Validation**: Focused `node --test test/inbox-filters.test.js test/mailbox.test.js test/radar-selection-transition.test.js` passed 61/61 across 8 suites with zero failures, skips, cancellations, or todos. `npm run build:renderer` passed in 574 ms with no warnings or errors. Exact `inbox-recent-filter-edit-workflow` browser acceptance passed 1/1 after a 521 ms build with zero unexpected external requests, page/console errors, desktop/mobile overflow failures, or desktop/mobile overlap failures.
- **Files changed by this repair**: `src/svelte/components/RadarView.svelte` and this Goose history entry. No filter, mailbox, style, prompt, backend, test, script, package, documentation, or configuration file was edited.

### 2026-09-08 - Add Item Scanner Selection Synchronization Repair
- **Root cause**: `AddTaskModal.svelte` read both the parent `scannerId` prop and local `selectedScannerId` inside one `$effect`. Selecting another scanner changed the local dependency, reran the effect, and immediately restored the original parent scanner before submit.
- **Repair**: The effect now initializes local scanner state from `scannerId` when `open` is true without reading `selectedScannerId`. A user selection no longer retriggers initialization, while every later closed-to-open transition still initializes from the current parent scanner scope. `RadarView.svelte` required no change.
- **Validation**: `npm run build:renderer` passed. Exact `FLIGHTDECK_BROWSER_SCENARIO=add-item-submit-persist-reload node scripts/test-radar-browser.js` passed 1/1 with zero unexpected external requests, page/console errors, overflow failures, or overlap failures. Focused `node --test test/radar-navigation.test.js test/radar-selection-transition.test.js` passed 33/33 with zero failures, skips, cancellations, or todos. Authorized product-file `git diff --check` passed, and VS Code reported no diagnostics in `AddTaskModal.svelte`.
- **Files changed by this repair**: `src/svelte/components/AddTaskModal.svelte` and this Goose history entry only. No parent, test, script, package, library, style, prompt/schema, documentation, or configuration file was edited.
