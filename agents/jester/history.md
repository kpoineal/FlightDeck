...
### 2026-09-09 — Electron Install Repair on Node 26
- With Node `v26.2.0`, Electron `41.0.3` postinstall exited 0 while leaving an incomplete `node_modules/electron/dist` plus no `path.txt` or `electron.exe`; `npm rebuild electron` and `npm ci --foreground-scripts` both reproduced the silent incomplete install.
- `@electron/get` resolved the checksum-validated Windows x64 archive, but Electron's `extract-zip` promise did not settle before Node exited. Synchronous extraction of that validated archive plus the standard `path.txt` marker restored the local shim and binary without changing either manifest.
- Verified `npm run build:renderer` exit 0, local shim and binary `v41.0.3`, and a healthy `npm start` with one visible `FlightDeck` window. FlightDeck intentionally hides on window close and remains in the tray, so validation cleanup must use the tray Quit/app quit path or exact repo-local process cleanup.
