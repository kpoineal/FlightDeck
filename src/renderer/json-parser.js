...
function sanitizeLikelyBrokenJson(input) {
  let output = '';
  let inString = false;
 ...
 * Returns an array of { index, url } objects.
 */
function extractFootnoteCitations(text) {
  if (!text) return [];
  const citations = [];
  const seen = new Set();
  // Match [N](url) patterns — standard markdown footnote references
  // Allow [^)]+ so URLs with line-break-induced spaces are still captured.
...
function injectFootnoteCitations(parsed, rawText) {
  ...
  if (Array.isArray(parsed.radarItems)) {
    ...
    if (emptyItems.length > 0) {
      // Strategy 1: If citation count matches item count, assign 1:1
      // Strategy 2: If more citations than items, distribute round-robin
      // Strategy 3: If fewer citations, assign sequentially
      let citationIdx = 0;
    ...
    // Also try to match citations to items that reference footnote numbers in text
    // e.g., if summary previously had [1] stripped, match by order
  ...
  for (const ledgerKey of ['iOwe', 'othersOweMe', 'silentThreads']) {
    ...
    // Ledger citations are less reliably mapped; skip 1:1 mapping
    // but if there are leftover citations, attach to entries without links
    // For now, just ensure the arrays exist
...
function parseWorkiqJson(answer, validator) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (validator(parsed)) {
        // Inject any footnote citations from the surrounding text
